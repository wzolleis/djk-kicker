    ...
    <header className="bg-black text-white sm:flex sm:items-center sm:justify-between sm:px-4 sm:py-3">
      <div className="flex items-center justify-between px-4 py-3 sm:p-0">
        ...
        <div className="sm:hidden">
          <button type="button" className="block hover:text-white focus:text-white focus:outline-none" onClick={() => setIsOpen(!isOpen)}>
      ...
      <nav className={`${menuItemClassName} px-2 pt-2 pb-4 sm:flex sm:p-0`}>
        ...
        <Form action="/logout" method="post" hidden={!isLoggedIn}>
          <button type="submit" className="block rounded px-2 py-1 font-semibold hover:bg-gray-800 hover:text-yellow-300">

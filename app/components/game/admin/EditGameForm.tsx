  ...
  return (
    <ContentContainer>
      <Form method={"post"} className={"flex flex-col gap-2"}>
        <InputWithLabel id={"name"} type={"text"} name={"name"} label={messages.adminEditGameForm.name} defaultValue={game.name} />
        <DateTimeInput name="gameTime" defaultValue={DateTime.fromJSDate(new Date(game.gameTime))} />
        <SelectWithLabel id={"location"} name={"location"} defaultValue={configuration.gameLocations[Number.parseInt(game.spielort)]} label={messages.adminEditGameForm.spielort}>

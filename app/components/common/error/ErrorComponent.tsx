  ...
  return (
    <>
      <div className={"flex h-full flex-col items-center gap-10"}>
        ...
        <div>
          <motion.div className={"font-unbounded-bold text-display-large"} variants={animationContainer} initial={"initial"} animate={controls}>
          ...
          <motion.div variants={animationContainer} initial={"initial"} animate={controls}>
            <p className={"text-center font-default-semibold text-title-medium text-gray-800"}>
              {errorMessage.split(" ").map((word, index) => (
                <motion.span className={"mr-1 inline-block"} key={index} variants={subtextCharacterAnimation}>
                  {word}
                </motion.span>
              ))}
            </p>
          ...
          <div className={"mt-5 flex w-full flex-col items-center gap-2"}>
            <button onClick={() => goBack()} type={"button"} className={"w-full"}>
              <DefaultButton>
                <p>Zurück</p>
              </DefaultButton>
            </button>

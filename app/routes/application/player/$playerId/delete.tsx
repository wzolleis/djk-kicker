...
export const action: ActionFunction = async ({params, request}: { params: Params, request: Request }) => {
    ...
    if (intent === 'abort') {
        console.log('>>>>>> delete player action - abort')
        return redirect(routeLinks.admin.users.home)
    } else if (intent === 'delete') {
        console.log('>>>>>> delete player action - delete')
        const player = await getPlayerById(playerId)
        invariant(!!player, "Spieler nicht gefunden")
...
const DeletePlayer = () => {
    ...
    return (
        <Form method={'post'}>
            <div className="flex flex-col bg-red-100 rounded-lg p-4 mb-4 text-sm text-red-700 md:w-1/4" role="alert">
                <div>

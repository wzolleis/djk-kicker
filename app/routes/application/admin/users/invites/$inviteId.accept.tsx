...
export const action: ActionFunction = async ({
    ...
    invariant(
        ...
        "Keine Einladung in der URL enthalten"
    );
    ...
    invariant(
        !!invitation,
        `Einladung mit der ID "${inviteId}" nicht gefunden`
    );
    const userByMail = await getUserByEmail(invitation.email);
    invariant(
        !userByMail,
        "Es existiert bereits ein Anwender für diese Mail-Adresse"
    );
    ...
    await updateAdminInvitation(inviteId, {
...
export const loader: LoaderFunction = async ({ params, request }) => {
    ...
    if (!token) {
        throw new Response("Ungültiges Token", {
            status: 400,
            statusText: `Ungültiges Token: "${token}"`,
        });
    }

    const adminToken = (await decryptEncryptedAdminToken(token))
        .payload as AdminToken;
    if (adminToken.email !== adminInvitation.email) {
        throw new Response("Das Token ist ungültig", {
            status: 400,
            statusText: "Das Token ist ungültig",
        });
    }
    const validUntil = DateTime.fromISO(adminToken.expires_at);
    if (validUntil.endOf("day") < DateTime.now()) {
        throw new Response("Das Token ist abgelaufen", {
            status: 400,
            statusText: "Das Token ist abgelaufen",
        });
    }
...
const InviteResponseView = () => {
    ...
    const validUntil = DateTime.fromJSDate(
        new Date(adminInvitation.expires_at)
    );
    return (
        <>
            <header>
                <PageHeader
                    title={messages.adminInviteUserResponseForm.title}
                />
            </header>
            <Form method={"post"} className="md:w-1/2">
                <main className={"flex flex-col gap-4"}>
                    <InputWithLabel
                        label={messages.adminInviteUserResponseForm.email}
                        type="email"
                        name={"email"}
                        id={"email"}
                        disabled={true}
                    ...
                    <InputWithLabel
                        ...
                        id={"validUntil"}
                        disabled={true}
                        defaultValue={dateUtils.dateToFormat({
                            value: validUntil,
                        })}
                    />
                    <InputWithLabel
                    ...
                    />
                    <InputWithLabel
                        label={messages.adminInviteUserResponseForm.password}
                        type="password"
                        name={"password"}
                        id={"password"}
                    ...
                    <InputWithLabel
                        label={
                            messages.adminInviteUserResponseForm.passwordRepeat
                        }
                        type="password"
                        name={"passwordRepeat"}
                        id={"passwordRepeat"}
...
export const CatchBoundary = () => {
    ...
    if (caught.status === 400 || caught.status === 404) {
        return (
            <div className="error-container">{`Huh? What the heck are u doing: ${caught.statusText}`}</div>
        );
    }
    ...
    return (
        <>
            <section>
                <h5
                    className={
                        "font-default-bold text-display-small tracking-tighter text-black"
                    }>{`Die Einladung kann nicht bearbeitet werden.`}</h5>
            </section>

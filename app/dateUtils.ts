...
const dateToFormat = ({value, options}: { value: DateTime, options?: { format: string } }): string => {
    return value.toFormat(options?.format ?? "dd.MM.yyyy")
}

const dateFromFormat = ({text, options}: { text: string, options?: { format: string } }): DateTime => {
    return DateTime.fromFormat(text, options?.format ?? "dd.MM.yyyy")
}
